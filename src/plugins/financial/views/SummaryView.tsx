...
function CashFlowBreakdown() {
  ...
  return (
    <div className="rounded-lg border bg-card p-5 h-full">
      <h3 className="text-sm font-semibold mb-1">Cash Flow Breakdown</h3>
      <p className="text-xs text-muted-foreground mb-4">Receivables vs. payables this month</p>
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            <span className="flex items-center gap-1.5 text-muted-foreground">
              <span className="h-2 w-2 rounded-full bg-red-500" />Paid
            </span>
...
function OverdueAlerts() {
  ...
  const items = [
    { label: 'Overdue receivable', value: summary?.overdueReceivableAmount ?? 0, count: summary?.overdueReceivableCount ?? 0, icon: ArrowUpRight, color: 'text-amber-500' },
    { label: 'Overdue payable', value: summary?.overduePayableAmount ?? 0, count: summary?.overduePayableCount ?? 0, icon: ArrowDownRight, color: 'text-red-500' },
  ]
  ...
  return (
    <div className="rounded-lg border bg-card p-5 h-full">
      ...
      {items.every((i) => i.count === 0) && (
        <p className="text-xs text-muted-foreground mt-3 pt-3 border-t">No overdue transactions</p>
      )}
    </div>
  )
}
    ...
    <div className="rounded-lg border bg-card p-5">
      ...
      {invoices.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Transactions will appear here once recorded</p>
        </div>
      ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          subtitle={summaryLoading ? 'Loading...' : 'All accounts'}
          icon={Wallet}
        ...
        <StatCard
          ...
          subtitle={`${summary?.overdueReceivableCount ?? 0} overdue`}
          icon={TrendingUp}
        ...
        <StatCard
          ...
          subtitle={`${summary?.overduePayableCount ?? 0} overdue`}
          icon={TrendingDown}
        ...
        <StatCard
          ...
          subtitle={`${(summary?.monthlyInflow ?? 0) >= (summary?.monthlyOutflow ?? 0) ? 'Positive' : 'Negative'} balance`}
          icon={BarChart3}
