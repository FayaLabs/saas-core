...
const STEPS = [
  { id: 'welcome', icon: Filter, title: 'Welcome to Sales & CRM', description: 'Track leads, manage your sales pipeline, and close more deals.' },
  { id: 'sources', icon: Globe, title: 'Lead Sources', description: 'Default sources (Instagram, WhatsApp, Google, etc.) are ready. Customize them in Settings.' },
  { id: 'pipeline', icon: Target, title: 'Sales Pipeline', description: 'Your default pipeline with 7 stages is set up. Customize stages and probabilities anytime.' },
]
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-xl border bg-card p-8 text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            {step > 0 && <button onClick={() => setStep(step - 1)} className="rounded-lg border px-4 py-2 text-sm font-medium hover:bg-muted/50 transition-colors">Back</button>}
            {isLast ? (
              <button onClick={onComplete} className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
                <Check className="h-4 w-4" /> Start selling
              </button>
            ) : (
          ...
          {step === 0 && <button onClick={onComplete} className="text-xs text-muted-foreground hover:text-foreground transition-colors">Skip setup</button>}
        </div>
      </div>
    </div>
  )
